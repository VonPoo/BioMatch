...
test:
	...
	python $(ROOT_DIR)/biomatch/analysis_scripts/00_extractSNPsfromVCF.py --help
...
#create snp sub k-mer fasta files (use updated Python script)
%_subKmers.fa: %_snps.vcf %_reference.fa
	python $(ROOT_DIR)/biomatch/analysis_scripts/00_extractSNPsfromVCF.py -v $< -f $*_reference.fa -s $(k) -k $(w) > $@
...
#filter repetitive sub k-mers & create concatenated sub k-mer fasta files (switch to Python version)
$(name)_n0.fa: $(name).sam
	python $(ROOT_DIR)/biomatch/analysis_scripts/filterRepetiveSNP.py $(name).sam $(name) $(w) $(k)
