...
class ExtractKmers:
    ...
    def _encodeKmer_strict(self, kmer):
        """Strict encoding that returns None for k-mers containing N"""
    ...
    def _encodeKmer_legacy(self, kmer):
        """Legacy encoding method - fast but will error on N bases"""
        fw_encode = {'A': 0, 'T': 1, 'C': 2, 'G': 3}
        rv_encode = {'A': 1, 'T': 0, 'C': 3, 'G': 2}
        fw = 0
        rv = 0
    ...
    def _checkVariant(self, base1, base2):
        """Check if variant is a transversion (A/T ↔ C/G)"""
    ...
    def _orderVariant(self, base1, base2):
        """Return false if ordering needs to be reversed"""
        if(base1 == "A" or base1 == "T"):
            return True
        else:
            return False
    ...
    def _hasNearbyN(self, fastaFile, chr, pos, distance):
        """Check if there is an 'N' base within 'distance' bp of the position"""
    ...
    def _detectNBases(self, fastaFile):
        """Quick scan to detect if any N bases exist in relevant regions"""
    ...
    def _extract_legacy(self, fastaFile):
        """Legacy fast extraction method - used when no N bases detected"""
